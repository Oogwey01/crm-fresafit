/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Solo lectura. La tabla de Pedidos ya pinta un semáforo de plazo ("se pasó el
   plazo" / "vence en horas"), pero solo lo ven los pedidos de Mercado Libre:
   `sales.envio_limite_despacho` únicamente la escribe esa sync. Para que TikTok
   entre al mismo semáforo hace falta saber QUÉ campos de SLA manda de verdad
   esta cuenta (región MX) — el tipo `OrdenTikTok` declara nueve campos y
   descarta el resto del payload, así que hoy nadie lo sabe.
   ...
   LO QUE ENCONTRÓ (tienda MX, 06/08/2026), y de donde sale el mapeo que hoy
   usa lib/tiktok/ventas.ts:
     * `rts_sla_time`   — 6/6 de las pendientes. Es el PLAZO DE DESPACHO: cae
                          siempre a las 23:59:59 hora de México (el día límite
                          completo), típicamente el día siguiente a la compra.
     * `rts_time`       — 10/10 de las AWAITING_COLLECTION / IN_TRANSIT /
                          DELIVERED, y en NINGUNA pendiente. Es la salida real.
     * `tts_sla_time`   — plazo de ENTREGA (3-5 días después), no de despacho.
     * `collection_due_time` = `cancel_order_sla_time` — el tope tardío de
                          recolección (11 días); no sirve para saber qué empacar.
     * `shipping_due_time` NO existe en esta API, pese a la documentación.
...
/* Misma firma que lib/tiktok/api.ts:52 — con el body dentro, que es lo que
   cambia respecto de los sondeos anteriores (search es POST). */
...
async function tt(metodo, path, { query = {}, body = null } = {}) {
  ...
  const res = await fetch(`${API}${path}?${new URLSearchParams(q)}`, {
    method: metodo,
    headers: {
      "x-tts-access-token": fila.access_token,
...
if (ordenes.length === 0) {
  console.log(
    "Sin pendientes en la ventana: no hay plazo vivo que mirar. Amplía DIAS_VENTANA.",
  );
  process.exit(0);
}
...
for (const o of ordenes.slice(0, A_DETALLE)) {
  ...
  const soloDetalle = Object.keys(det).filter((k) => !enListado.has(k));
  const enDetalle = new Set(Object.keys(det));
  const soloListado = Object.keys(o).filter((k) => !enDetalle.has(k));
  console.log("Claves solo en el DETALLE:", soloDetalle.join(", ") || "—");
  console.log("Claves solo en el LISTADO:", soloListado.join(", ") || "—");
}
...
function inventariar(lista) {
  ...
  for (const o of lista) {
    for (const [k, v] of Object.entries(o)) {
      ...
      c.veces += 1;
      if (c.ejemplo === undefined || c.ejemplo === null || c.ejemplo === 0)
        c.ejemplo = v;
      conteo.set(k, c);
    }
  }
...
const enMX = (seg) =>
  typeof seg === "number" && seg > 0
    ? new Date(seg * 1000).toLocaleString("es-MX", {
...
for (const [k, c] of [...conteo.entries()].sort(
  (a, b) => b[1].veces - a[1].veces,
)) {
  console.log(
    `${k.padEnd(24)} en ${String(c.veces).padStart(3)}/${ordenes.length}  ej: ${String(c.ejemplo).padEnd(12)} → ${enMX(c.ejemplo)}`,
  );
}
...
console.log(
  "Compara estas horas con el 'Enviar antes de' que muestra el Seller",
);
...
for (const o of ordenes.slice(0, 10)) {
  ...
  for (const k of claves)
    console.log(
      `    ${k.padEnd(24)} ${String(o[k] ?? "—").padEnd(12)} ${enMX(o[k])}`,
    );
  console.log("");
}
...
/* El semáforo necesita dos datos: el plazo y la salida real (sin ella, un
   pedido despachado a tiempo se quedaría marcado como pendiente para siempre).
   En las órdenes AWAITING_SHIPMENT ese campo todavía no existe, así que hay que
   ir a mirar órdenes que ya pasaron por ahí. */
...
for (const estado of YA_DESPACHADAS) {
  ...
  for (const [k, c] of [...inv.entries()].sort(
    (a, b) => b[1].veces - a[1].veces,
  )) {
    console.log(
      `${k.padEnd(24)} en ${String(c.veces).padStart(3)}/${lista.length}  ej: ${String(c.ejemplo).padEnd(12)} → ${enMX(c.ejemplo)}`,
    );
  }
  /* Claves que NO estaban en las pendientes: ahí vive la salida real. */
