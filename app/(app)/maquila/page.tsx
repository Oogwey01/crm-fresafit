...
import { COLUMNAS_PEDIDO_MAQUILA, cargarCalendarioMaquila, listarCostosMaquila } from "@/lib/maquila/consultas";
...
export default async function MaquilaPage() {
  ...
  const pedidos = await traerTodo<PedidoMaquila>((desde, hasta) =>
    supabase
  ...
  if (esMaquilero(rol)) {
    return <VistaMaquilero pedidos={pedidos} hoy={hoy} nombre={perfil?.nombre ?? "Eduardo"} />;
  }
  ...
  const [fichas, productos, calendario, costos] = await Promise.all([
